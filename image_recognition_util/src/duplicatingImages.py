...
# test if the given path is a usable directory
...
# test if the given path is a valid directory and creates it if necessary
...
def change_scale(scale_image, bbox, new_path, min, max):
...
if __name__ == "__main__":
    ...
    parser.add_argument('background_dir', type=readable_dir, help='sys-path to the background images')
    ...
    image_path = args.imageset_dir
    save_path = args.save_image_dir
    bg_path = args.background_dir

    num_light = args.lighting
    num_scale = args.scale
    num_blur = args.blur
    num_rotate = args.rotate

    print("change image (light, scale, blur, rotate): {}x{}x{}x{} times".format(num_light, num_scale, num_blur,
                                                                                num_rotate))
    ...
    for dirname, dirnames, filenames in os.walk(image_path):
        for filename in filenames:
            ...
            if not (os.path.isfile(label_path) and os.path.isfile(
                    mask_path)):  # ignore image if label or mask doesnt exist
                continue
